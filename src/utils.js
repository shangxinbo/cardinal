...
function fileLog(content, path = DEFAULT_PATH) {
...
exports.createSafeAfterHandler = function(logger, next) {
  let numFlushes = 0;
  let numFlushed = 0;
...
exports.closeSilently = function(server) {
  if (server) {
    try {
      server.close();
    } catch (e) {
...
function sendDgram(socket, data, ...args) {
  socket.send(data, 0, data.length, ...args);
}

exports.writeOrPause = function(fromCon, toCon, data) {
